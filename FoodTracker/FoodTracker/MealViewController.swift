...
//        var studyTime:NSNumber = self.studyTime.text as! NSNumber
    ...
    func read() {
        ...
        do{
            ...
            myTitle = NSArray() as! [String]
            //nilが入るかもしれないのでasに?をつける。
            for result: AnyObject in fetchResults {
                
                let title:String? = result.value(forKey: "coffeeName") as? String
                let saveDate: Date? = result.value(forKey: "date") as? Date
                ...
                print("name:\(title) saveDate:\(saveDate) studyTime:\(studyTime) image:\(img) rating:\(rating)")
